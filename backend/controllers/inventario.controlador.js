...
controller.deleteInventario = async (req, res) => {
  try {
    const inventarioEliminado = await inventarioModel.findByIdAndDelete(req.params.id);
    if (!inventarioEliminado) return res.status(404).json({ message: 'Producto no encontrado' });
